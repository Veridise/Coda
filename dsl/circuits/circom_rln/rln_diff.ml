...
let t_root identity_secret user_message_limit path_elements identity_path_index
    =
  tfq
    (qeq nu
       (u_hasher
...
let rln =
  Circuit
    ...
    ; outputs=
        ...
        ; ( "root"
          , t_root identity_secret user_message_limit path_elements
              identity_path_index )
        ; ( "nullifier"
    ...
    ; body=
        elet "identityCommitment"
          ...
          (elet "rateCommitment"
             (call "Poseidon"
                [ z2
                ; const_array tf [v "identityCommitment"; v "userMessageLimit"]
                ] )
             (elet "root"
                ...
                (elet "rangeCheck"
                   (call "RangeCheck"
                      [v "LIMIT_BIT_SIZE"; v "messageId"; v "userMessageLimit"] )
                   (elet "a1"
                      (call "Poseidon"
                         [ z3
                         ; const_array tf
