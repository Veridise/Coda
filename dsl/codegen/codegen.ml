...
type r1cs = 
  | R1CS of (string * coeff) list * (string * coeff) list * (string * coeff) list
  [@@deriving show]
...
let rec reify_expr (prefix: string) (g: gamma) (b: beta) (d: delta) (a: alpha) (e: expr) : gamma * beta * alpha * expr = 
  match e with
  | NonDet -> 
    (* generate a fresh var for it *)
    let x = fresh_var prefix () 
    in ((x, Var x) :: g, x :: b, a, Var x)
  | CPrime 
  | CPLen 
  | Const _ -> (g, b, a, e)
  | Binop (ty, op, e1, e2) ->
  ...
  | Call (c_name, args) ->
    ...
    | Some c ->
      ...
      (match out_vars with
      | [out_var] -> (g, b', a', Var out_var) (* use original gamma *)
      | _ -> failwith "Multiple outputs not supported")
    | None -> failwith ("No such circuit: " ^ c_name))
  | _ -> failwith (Format.sprintf "Codegen unavailable for expression %s" (show_expr e))

and codegen_stmt prefix (g: gamma) (b: beta) (d: delta) (a: alpha) (s: stmt) : gamma * beta * alpha =
  ...
  | SAssert (e1, e2) ->
    ...
    let (g'', b'', a'', e2') = reify_expr prefix g' b' d a' e2 in
    (g'', b'', a'' @ [qeq e1' e2'])
  | _ -> todos "codegen_stmt"

and codegen_circuit (args: expr list) (g: gamma) (b: beta) (d: delta) (a: alpha) (c: circuit) : gamma * beta * alpha * string list =
  match c with
  | Circuit {name; inputs; outputs; dep; body} ->
    let (g', b', a', args') = List.fold_left (fun (g, b, a, args) e ->
      let (g', b', a', e') = reify_expr name g b d a e in
      (g', b', a', args @ [e'])) (g, b, a, []) args in
    (* output: NonDet *)
    let g1 = init_gamma c args' @ g' in
    let out_vars = ref [] in
    let (g'', b'', a'') = List.fold_left (fun (g, b, a) (x,_) ->
      let x' = fresh_var name () in
      out_vars := !out_vars @ [x'];
      ((x, Var x') :: g, x' :: b, a)) (g1, b', a') (List.rev outputs) in
    let (g''', b''', a''') = List.fold_left
