...
let filter_trivial =
  List.filter (function
    | Subtype (_, _, TRef (t1, _), TRef (t2, QTrue)) ->
        t1 <> t2
    | Subtype (_, _, TRef (t1, QAnd (q1, _)), TRef (t2, q2)) ->
        q1 <> q2
    | Subtype (_, _, TRef (t1, QAnd (_, q1)), TRef (t2, q2)) ->
        q1 <> q2
...
let rec synthesize (d : delta) (g : gamma) (a : alpha) (e : expr) :
    typ * cons list =
  ...
  and f' (e : expr) : typ * cons list =
    ...
    | NonDet ->
        (tfq QTrue, [])
    | CPrime ->
        (TRef (TInt, QExpr (eq nu CPrime)), [])
    | CPLen ->
        (TRef (TInt, QExpr (eq nu CPLen)), [])
    | Const c ->
        ...
        let t =
          ...
          | _ ->
              failwith
                (Format.sprintf "synthesize: invalid constant %s" (show_expr e))
        in
        (t, [])
    | Var v ->
        let t =
    ...
    | Boolop (op, e1, e2) -> (
        ...
        match (tb1, tb2) with
        | TBool, TBool ->
            (re tb1 (eq nu e), cs1 @ cs2)
        | _ ->
            failwith
              (Format.sprintf "Boolop: Invalid operand type %s and %s"
                 (show_tyBase tb1) (show_tyBase tb2) ) )
    | Comp (op, e1, e2) -> (
        (* TODO: reflect *)
        ...
        | Leq | Lt -> (
          ...
          | TInt, TInt ->
              res
          | _ ->
              failwith "Comp: Cannot compare non-integers for inequality" )
        | _ ->
            if tb1 = tb2 then res
            else
              failwith
                ("Comp: Unequal types " ^ show_tyBase tb1 ^ show_tyBase tb1) )
    | Not e' -> (
        ...
        | _ ->
            failwith ("Opp: Invalid operand type " ^ show_tyBase tb) )
    | Call (c_name, args) -> (
    ...
    | Sum {s; e= e'; body= b} -> (
        ...
        let b' =
          ...
          | _ ->
              failwith "Sum's body must be Lam"
        in
    ...
    | TGet (e, n) -> (
        ...
        | TTuple ts | TDProd (ts, _, _) ->
            if 0 <= n && n < List.length ts then (List.nth ts n, cs)
            else failwith "Tuple access out of bounds"
        | _ ->
            failwith "Synthesize: expect tuple type" )
    | ArrayOp (Get, [e1; e2]) -> (
        let t1, cs1 = f e1 in
        match t1 with
        | TArr (t, q, el) ->
...
and synthesize_app (d : delta) (g : gamma) (a : alpha) (t : typ) (es : expr list)
    : typ * cons list =
  match es with
  | [] ->
      (t, [])
...
and check (d : delta) (g : gamma) (a : alpha) (e : expr) (t : typ) : cons list =
  ...
  | DPDestr (e1, xs, e2), t2 ->
      ...
      if List.length ts = List.length xs then
        ...
        cs1 @ cs2
      else failwith (Format.sprintf "DPDestr: xs and ts have different lengths")
  | _ ->
...
let typecheck_stmt (d : delta) (g : gamma) (a : alpha) (s : stmt) :
    gamma * alpha * cons list =
  match s with
  | SSkip ->
      (g, [], [])
  | SLet (x, e) ->
...
let typecheck_circuit (d : delta) (c : circuit) : cons list =
  ...
  | Circuit {name; inputs; outputs; dep; body} ->
      ...
      let dep_cons =
        dep |> Option.map (fun q -> CheckCons (g, a, q)) |> Option.to_list
      in
