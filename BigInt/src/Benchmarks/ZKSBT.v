...
Proof.
  unwrap_C; intuit; subst; try fqsatz;
    try (left; fqsatz);
    try (right; fqsatz).
Admitted.
...
Proof.
  intuit; subst; try lia.
  apply H7. Signed.solve_to_Z.
Qed.
...
Lemma getIdenState_obligation1: forall (claimsTreeRoot : F) (revTreeRoot : F) (rootsTreeRoot : F) (v : (list F)), True -> True -> True -> Forall (fun x0 => True) v -> True -> (((((True /\ ((v!0%nat) = claimsTreeRoot)) /\ ((v!1%nat) = revTreeRoot)) /\ ((v!2%nat) = rootsTreeRoot)) /\ ((length v) = 3%nat)) -> ((length v) = 3%nat)).
Proof. hammer. Qed.
...
Lemma cutState_obligation4: forall (_in : F) (n2b : (list F)) (d : (list F)) (v : (list F)), True -> Forall (fun x3 => ((x3 = 0%F) \/ (x3 = 1%F))) n2b -> (((as_le_f n2b) = _in) /\ ((length n2b) = 256%nat)) -> Forall (fun x4 => True) d -> ((d = (skipn 40%nat n2b)) /\ ((length d) = ((length n2b) - 40%nat)%nat)) -> Forall (fun x5 => True) v -> True -> ((((v = (skipn 40%nat n2b)) /\ ((length v) = ((length n2b) - 40%nat)%nat)) /\ (v = d)) -> ((length v) = 216%nat)).
Proof. hammer. Qed.
