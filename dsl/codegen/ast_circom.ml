...
type varParams = ConstInt of int | VarInt of string

type varType =
  | Array of varParams list
    (* var x[123][456][789]] ==> Array [ConstInt 123;ConstInt 456;ConstInt 789] *)
  | Expr (* var x; *)
...
type varDecl = Input of varType | Output of varType | Signal of varType
...
let print_varParams (v : varParams) : unit =
  match v with ConstInt i -> print_int i | VarInt s -> print_string s
...
let rec print_expr (e : expr) : unit =
  ...
  | Get (e, v) ->
      ...
      print_string ")"
  | Opp e ->
      print_string "Opp (" ; print_expr e ; print_string ")"
  | Add (e1, e2) ->
      print_string "Add (" ;
      print_expr e1 ;
      print_string ", " ;
  ...
  | Mul (e1, e2) ->
      ...
      print_string ")"
  | Exp2 e1 ->
      print_string "Exp2 (" ; print_expr e1 ; print_string ")"
  | Field (e, s) ->
      print_string "Field (" ;
      print_expr e ;
      print_string ", " ;
...
let print_varDecl (v : varDecl) : unit =
  match v with
  | Input t ->
      print_string "Input (" ; print_varType t ; print_string ")"
  | Output t ->
      print_string "Output (" ; print_varType t ; print_string ")"
  | Signal t ->
      print_string "Signal (" ; print_varType t ; print_string ")"
...
let rec print_stmts (s : stmts) : unit =
  ...
  | [] ->
      ()
  | s :: s' ->
      print_stmt s ; print_string ";" ; print_stmts s'
...
let rec print_program (p : program) : unit =
  ...
  | [] ->
      ()
  | c :: c' ->
      print_circuit c ; print_string ";" ; print_program c'
...
let string_of_dsl_coq c =
  ...
  let print_varType = function
    | Array l ->
        print "F" ;
        List.iter (fun x -> print "^" ; print_varParams x) l (* todo: nested *)
    | Expr ->
        print "F"
  in
  let rec print_expr = function
    ...
    | Var s ->
        print s
    | Get (e1, e2) ->
        print_expr e1 ; print "[" ; print_varParams e2 ; print "]"
    | Opp e ->
        print "(" ; print "-" ; print_expr e ; print ")"
    | Add (e1, e2) ->
        print "(" ; print_expr e1 ; print " + " ; print_expr e2 ; print ")"
    | Sub (e1, e2) ->
        print "(" ; print_expr e1 ; print " - " ; print_expr e2 ; print ")"
    | Mul (e1, e2) ->
        print "(" ; print_expr e1 ; print " * " ; print_expr e2 ; print ")"
    | Exp2 e1 ->
        print "(" ; print " 2 ^ " ; print_expr e1 ; print ")"
    | Field (e, s) ->
        print_expr e ; print "." ; print s
  in
  let rec print_loop_decl = function
    ...
    | (s, _, _) :: [] ->
        print s
    | (s, _, _) :: l ->
        print s ; print ", " ; print_loop_decl l
  in
  let rec print_loop_decl_init = function
    ...
    | (_, s, _) :: [] ->
        print_expr s
    | (_, s, _) :: l ->
        print_expr s ; print ", " ; print_loop_decl_init l
  in
  let rec print_loop_decl_iter = function
    ...
    | (_, _, s) :: [] ->
        print_expr s
    | (_, _, s) :: l ->
        print_expr s ; print ", " ; print_newline () ; print_loop_decl_iter l
  in
  let rec print_stmts = function
    ...
    | h :: [] ->
        let rec print_stmt = function
          | Constraint (e1, e2) ->
              print_expr e1 ; print " = " ; print_expr e2
          | Iter (_, s2, l, s3) ->
              print "let '(" ;
              print_loop_decl l ;
              print ", _C) :=" ;
              ...
              print ") " ;
              print "(" ;
              print_loop_decl_init l ;
              print ", True)) in _C /\\"
          | Call (s1, s2) ->
              print "exists (" ; print s1 ; print ": " ; print s2 ; print ".t),"
          | Map _ ->
              ()
          | Map2 _ ->
              ()
  ...
  let print_circuit = function
    | Template (s, l1, l2, l3) ->
        ...
        List.iter
          (fun x ->
            print "Context {" ; print x ; print " : nat}." ; print_newline () )
          l1 ;
        ...
        List.iter
          (fun (x, y) ->
            print_str_decl_record (x, y) ;
            print "; " )
          l2 ;
        print "_cons: " ;
        List.iter (fun (x, y) -> print_str_decl_record1 (x, y)) l2 ;
        print "cons " ;
        List.iter (fun (x, _) -> print x ; print " ") l2 ;
        print "}." ;
        print_newline () ;
        print_newline () ;
        print "End " ;
...
let is_zero =
  Template
    ( "IsZero"
    , []
    , [("_in", Input Expr); ("out", Output Expr); ("_inv", Signal Expr)]
...
let is_equal =
  Template
    ( "IsEqual"
    , []
    , [("_in", Input (Array [ConstInt 2])); ("out", Output Expr)]
    , [ Call ("isz", "IsZero")
      ; Constraint
          ( Sub (Get (Var "_in", ConstInt 1), Get (Var "_in", ConstInt 0))
          , Field (Var "isz", "_in") )
...
let num2bits =
  Template
    ( "Num2Bits"
    , ["n"]
    , [("_in", Input Expr); ("out", Output (Array [VarInt "n"]))]
    , [ Iter
          ( "i"
          , "n"
          , [ ( "lc1"
              , Sig 0
              , Add (Var "lc1", Mul (Get (Var "out", VarInt "i"), Var "e2")) )
            ; ("e2", Sig 1, Add (Var "e2", Var "e2")) ]
          , Constraint
              ( Mul
                  ( Get (Var "out", VarInt "i")
                  , Sub (Get (Var "out", VarInt "i"), Sig 1) )
              , Sig 0 ) )
...
let modsum =
  Template
    ...
    , [ Call ("n2b", "Num2Bits")
      ...
      ; Constraint
          ( Var "sum"
          , Sub (Add (Var "a", Var "b"), Mul (Var "carry", Exp2 (Var "n"))) ) ]
    )
...
let modsumthree =
  Template
    ...
    , [ Call ("n2b", "Num2Bits")
      ; Constraint
          (Field (Var "n2b", "_in"), Add (Var "a", Add (Var "b", Var "c")))
      ; Constraint
          ( Var "carry"
          , Add
              ( Get (Field (Var "n2b", "out"), VarInt "n")
              , Mul (Sig 2, Get (Field (Var "n2b", "out"), VarInt "n+1")) ) )
      ; Constraint
          ( Var "sum"
          , Sub
              ( Add (Var "a", Add (Var "b", Var "c"))
              , Mul (Var "carry", Exp2 (Var "n")) ) ) ] )
...
let modsub =
  Template
    ...
    , [ Call ("lt", "LessThan")
      ...
      ; Constraint
          ( Var "out"
          , Add (Mul (Var "borrow", Exp2 (Var "n")), Sub (Var "a", Var "b")) )
      ] )
...
let modsubthree =
  Template
    ...
    , [ Call ("lt", "LessThan")
      ; Constraint (Get (Field (Var "lt", "_in"), ConstInt 0), Var "a")
      ; Constraint
          (Get (Field (Var "lt", "_in"), ConstInt 1), Add (Var "b", Var "c"))
      ; Constraint (Field (Var "lt", "out"), Var "borrow")
      ; Constraint
          ( Var "out"
          , Add
              ( Mul (Var "borrow", Exp2 (Var "n"))
              , Sub (Var "a", Add (Var "b", Var "c")) ) ) ] )
