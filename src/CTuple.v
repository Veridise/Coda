...
Definition from_list'_length {T} (y:T) (xs:list T) :  tuple' T (length xs).
Proof. induction xs; simpl; auto. Defined.
...
Definition tforall {n} : tuple T n -> Prop :=
  match n return tuple T n -> Prop with
  | O => fun _ => True
  | S n' => fun x => tforall' x
  end.

Lemma tforall'_Forall: forall n (xs: tuple' T n), tforall' xs <-> List.Forall P (to_list' n xs).
Proof.
...
Proof.
  intros. destruct n; split; unfold tforall; 
  try apply tforall'_Forall; simpl; firstorder.
Qed.
(* 
...
Fixpoint nth_Default {A m} `{Default A} n : tuple A m -> A :=
  match m, n with
  | O, _ => fun _ => default
  | S m', O => hd
  | S m', S n' => fun x => nth_Default n' (tl x)
  end.
...
Lemma nth_Default_to_list' {A} `{H: Default A} n (xs : tuple' A n):
  forall i, List.nth_default default (to_list' n xs) i = @nth_Default A (S n) H i xs.
Proof using Type.
...
Qed.
Lemma nth_Default_to_list {A} `{H: Default A} n (xs : tuple A n):
  forall i, List.nth_default default (to_list _ xs) i = nth_Default i xs.
Proof using Type.
  destruct n; cbv [CTuple.tuple CTuple.to_list] in *.
