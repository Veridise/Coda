...
Proof. 
  ...
  - (* a + b < p, so (a+b) mod p = a + b *)
    ...
    assert (Huf: uf = 0%F) by admit.
    assert (Hsub: [|sub[:k0]|n]= [|sub|n] ) by admit.
    rewrite Hsub.
    apply f_equal with (f:=F.to_Z) in Huf. autorewrite with F_to_Z in Huf; try lia. rewrite Huf in *.
    replace ([|p'|n]) with 0 in * by admit.
    simplify.
