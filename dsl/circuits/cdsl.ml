...
let eassert_equal x y =
  ascribe
    (match_with (call "AssertEqual" [x; y]) [] (dpunit (qeq x y)))
    (tdunit (qeq x y))

let c_assert_equal_test =
  Circuit
    { name= "AssertEqualTest"
    ; inputs= [("x", tf); ("y", tf)]
    ; outputs= []
    ; dep= None
    ; body= [slet "u" (eassert_equal x y)] }
...
let c_assert_binary =
  Circuit
    ...
    ; body= [slet "u" (eassert_binary x)] }

let check_c_assert_binary = typecheck_circuit d c_assert_binary

let assert_binary x =
  ascribe
    (match_with (call "AssertBinary" [x]) [] (dpunit (q_is_binary x)))
    (tdunit (q_is_binary x))
