...
Lemma BigSub_obligation14: forall (n : nat) (k : nat) (a : (list F)) (b : (list F)) (i : nat) (s'_br' : (list F) * F) (br' : F) (s' : (list F)) (_u0 : (list F) * F) (ai : F) (bi : F) (s_br : F * F) (br : F) (s : F) (_u1 : F * F), ((n <= (C.k - 2%nat)%Z) /\ ((1%nat <= n) /\ True)) -> (1%nat <= k) -> Forall (fun x97 => ((^ x97) <= ((2%nat ^ n)%Z - 1%nat)%Z)) a -> ((length a) = k) -> Forall (fun x98 => ((^ x98) <= ((2%nat ^ n)%Z - 1%nat)%Z)) b -> ((length b) = k) -> (i < k) -> match s'_br' with (x100,x101) => Forall (fun x99 => ((^ x99) <= ((2%nat ^ n)%Z - 1%nat)%Z)) x100 end -> match s'_br' with (x100,x101) => ((length x100) = i) end -> match s'_br' with (x100,x101) => (((x101 = 0%F) \/ (x101 = 1%F)) /\ (((x101 = 1%F) -> ((as_le n (a[:i])) < (as_le n (b[:i])))) /\ ((x101 = 0%F) -> ~((as_le n (a[:i])) < (as_le n (b[:i])))))) end -> match s'_br' with (x100,x101) => ((as_le n x100) = (((as_le n (a[:i])) - (as_le n (b[:i])))%Z + ((2%nat ^ n)%Z * (^ x101))%Z)%Z) end -> (br' = (snd s'_br')) -> Forall (fun x102 => True) s' -> (s' = (fst s'_br')) -> match _u0 with (x104,x105) => Forall (fun x103 => True) x104 end -> match _u0 with (x104,x105) => True end -> match _u0 with (x104,x105) => True end -> match _u0 with (x104,x105) => ((s'_br' = s'_br') /\ True) end -> (ai = (a!i)) -> (bi = (b!i)) -> match s_br with (x106,x107) => True end -> match s_br with (x106,x107) => (((x107 = 0%F) \/ (x107 = 1%F)) /\ (((x107 = 1%F) -> ((^ ai) < (^ (bi + br')%F))) /\ ((x107 = 0%F) -> ~((^ ai) < (^ (bi + br')%F))))) end -> match s_br with (x106,x107) => (x106 = ((((x107 * 2%F)%F ^ n)%F + ai)%F - (bi + br')%F)%F) end -> (br = (snd s_br)) -> (s = (fst s_br)) -> match _u1 with (x108,x109) => True end -> match _u1 with (x108,x109) => True end -> match _u1 with (x108,x109) => ((s_br = s_br) /\ True) end -> (True -> ((as_le n (s' ++ (s :: nil))) = (((as_le n (a[:(i + 1%nat)%nat])) - (as_le n (b[:(i + 1%nat)%nat])))%Z + ((2%nat ^ n)%Z * (^ br))%Z)%Z)).
Proof. Admitted.
